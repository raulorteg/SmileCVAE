...
def main():
    ...
    cvae = CVAE(input_size=input_shape, latent_size=600).to(device)

    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(cvae.parameters(), lr=0.00002)
    train_loss_history = []
    num_epochs = 1000
    cvae.train()
    for epoch in range(num_epochs):
        ...
        grid.save(f"results/cvae/iter_{epoch}.png")

        if epoch == 0:
            images = [Image.fromarray(255*img.detach().numpy()[0]).convert("L") for img in x]
            grid = image_grid(images, 4, 8)
            grid.save(f"results/cvae/original.png")
        ...
        torch.save({
                    ...
                    'loss': train_loss,
                    }, "model_savepoints/cvae/model.pt")
