...
def sample_across_axis(axis, RESIZE, LATENT_SIZE):
    ...
    samples the latent space uniformly only varying the [axis]th element in the space defined by the linear transformation performed by PCA,
    leaving the rest of the latent values constant to 0 and decodes this latent samples into images. This is done to attempt to visualize what each
    ...
    latent = np.genfromtxt("results/latent.txt")
    pca = PCA()
    transformed_latent = pca.fit_transform(latent)
    xhat = []
    for value in np.linspace(-1.2,1.2,32):
        z = 0.0 * z_c
        z[0][axis] = value
        z[0][:-1] = torch.from_numpy(pca.inverse_transform(z[0][:-1].numpy()))
    ...
    grid.save(f"results/axis/pca_axis_{axis}.png")
    
if __name__ == "__main__":

    import argparse
    # parsing user input
    # example: python sample_across_pca_axis.py --axis=0 --resize=50 --latent=50
